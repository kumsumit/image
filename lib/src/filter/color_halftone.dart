...
Image colorHalftone(
  ...
  for (final frame in src.frames) {
    ...
    for (final p in frame) {
      ...
      cmyC = (cmyC * 10 - 3 + pattern(x, y, cx, cy, angle + 0.26179)).clamp(
        0,
        1,
      );
      cmyM = (cmyM * 10 - 3 + pattern(x, y, cx, cy, angle + 1.30899)).clamp(
        0,
        1,
      );
      cmyY = (cmyY * 10 - 3 + pattern(x, y, cx, cy, angle)).clamp(0, 1);
      cmyK = (cmyK * 10 - 5 + pattern(x, y, cx, cy, angle + 0.78539)).clamp(
        0,
        1,
      );
